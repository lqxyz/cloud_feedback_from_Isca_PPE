...
def sel_from_subsidence_region_tm(ds, slat=-30, nlat=30, ocn=True,
    ...
    if ocn:
        ...
        ind_low = ind_low & (ds.mask == 0)

    return ind_low

def sel_cu_sc_from_subsidence_region_tm(ds, slat=-30, nlat=30, ocn=True,
    ...
    lts = ds.lts.mean('time')
    eis = ds.eis.mean('time')
    lats = ds.lat

    if 'LTS' in method.upper():
        w0 = 10 # hPa/day
        ind_low = (w500 >= w0) & (w700 >= w0) & (lats >= slat) & (lats <= nlat)
        # select Cumulus and Stratocumulus
        #T0 = 20 #18.5  # K
...
if __name__ == '__main__':
    ...
    #cld_type_arr = ['Cu', 'Sc']
    cld_type = sys.argv[1] # Cu or Sc
    proxy = sys.argv[2].upper() # ELF, or EIS
    T0 = float(sys.argv[3])
    if len(sys.argv)>4:
        num_proxy = float(sys.argv[4])
    else:
    ...
    #else:
    #    method = 'EIS'
    method = sys.argv[5].upper() # ELF, or EIS
    print('cloud type, proxy, T0, proxy, method', cld_type, proxy, T0, num_proxy, method)

    #var_names = ['SST', 'EIS', 'Tadv', r'RH$_{700}$', r'$\omega_{700}$']
    #var_names = ['SST', 'ELF', 'Tadv', 'RH700', 'omega700']
    if num_proxy > 2:
    ...
    for tbl_var_nm in tbl_names:
        ...
        for kk, exp_grp in enumerate(exp_grps):
            ...
            ind_Cu_ocn_ctrl, ind_Sc_ocn_ctrl = sel_cu_sc_from_subsidence_region_tm(ds_arr[0],
                    slat=-35, nlat=35, ocn=True, land_mask_dir=lsm_dir, T0=T0, method=method)
            ind_Cu_ocn_perturb, ind_Sc_ocn_perturb = sel_cu_sc_from_subsidence_region_tm(ds_arr[1], 
                    slat=-35, nlat=35, ocn=True, land_mask_dir=lsm_dir, T0=T0, method=method)

            #ind_Cu_arr = [ind_Cu_ocn_ctrl, ind_Cu_ocn_perturb]
            #ind_Sc_arr = [ind_Sc_ocn_ctrl, ind_Sc_ocn_perturb]
            if 'Cu' in cld_type:
                ind_low_ocn_ctrl, ind_Sc_ocn_ctrl = sel_cu_sc_from_subsidence_region_tm(ds_arr[0],
                        slat=-35, nlat=35, ocn=True, land_mask_dir=lsm_dir, T0=T0, method=method)
                ind_low_ocn_perturb, ind_Sc_ocn_perturb = sel_cu_sc_from_subsidence_region_tm(ds_arr[1], 
                        slat=-35, nlat=35, ocn=True, land_mask_dir=lsm_dir, T0=T0, method=method)
            if 'Sc' in cld_type:
                ind_Cu_ocn_ctrl, ind_low_ocn_ctrl = sel_cu_sc_from_subsidence_region_tm(ds_arr[0],
                        slat=-35, nlat=35, ocn=True, land_mask_dir=lsm_dir, T0=T0, method=method)
                ind_Cu_ocn_perturb, ind_low_ocn_perturb = sel_cu_sc_from_subsidence_region_tm(ds_arr[1], 
                        slat=-35, nlat=35, ocn=True, land_mask_dir=lsm_dir, T0=T0, method=method)
        ...
        file_name = P(dt_dir, tbl_var_nm+'_CCF_ctrl_'+cld_type+'_'+proxy+'.csv')
        ...
        file_name = P(dt_dir, tbl_var_nm+'_CCF_perturb_'+cld_type+'_'+proxy+'.csv')
        ...
        if not CCF_change_calculated:
            ...
            file_name = P(dt_dir, 'CCF_change_perK_'+cld_type+'_'+proxy+'.csv')
            pdtbl = pd.DataFrame(data=CCF_chg_tbl, index=exp_grps, columns=var_names)
            pdtbl.to_csv(file_name, header=True, index=True, float_format="%.8f")
            print(file_name, 'saved.')

        file_name = P(dt_dir, tbl_var_nm+'_chg_per_K_'+cld_type+'_'+proxy+'.csv')
        pdtbl = pd.DataFrame(data=var_chg_tbl, index=exp_grps, columns=var_names)
        pdtbl.to_csv(file_name, header=True, index=True, float_format="%.8f")
        print(file_name, 'saved.')

        file_name = P(dt_dir, tbl_var_nm+'_chg_per_K_real_'+cld_type+'_'+proxy+'.csv')
