...
import cmaps
from bin_clisccp import get_percentile
...
def select_clisccp_data(ds_m, bin_data_dict, ds_mask, bins,
      ...
      var_names=['clisccp']):
    var_names1 = var_names + ['area_weight']
    for vn in var_names1:
        if not vn in bin_data_dict.keys():
            bin_data_dict[vn] = ds_m[vn]

    coords = {}
    dims = ds_m[var_names1[0]].dims #['time', 'tau7', 'pres7', 'lat', 'lon']
    try:
        tau_dim = 'tau7'
        tau7 = ds_m[tau_dim]
        pres7 = ds_m.pres7
    ...
    if grp_time_var is not None:
        ...
        for i in range(ntime):
            ...
            for var in ds.variables:
                if len(ds[var].shape) == 5:
                    ...
                    grouped_t = g_w.transpose(tau_dim, 'pres7', bin_var_nm+'_bins')
                    ds_bin_mean[var][i,:,:,:] = grouped_t
                if 'area_weight' in var:
                    g_s = grouped.sum()
        ...
        for var in ds.variables:
            if len(ds[var].shape) == 5:
                ...
                g_w = grouped.apply(weight_avg)
                grouped_t = g_w.transpose(tau_dim, 'pres7', bin_var_nm+'_bins')
                ds_bin_mean[var] = grouped_t
            if 'area_weight' in var:
                ds_bin_mean['area_sum'][:] = grouped.sum().variables.get('area_weight')
